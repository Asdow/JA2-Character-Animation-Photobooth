...
FOR /F "tokens=* delims=" %%x in (palettes.txt) DO (
...
set /a animIndex=0
FOR /F "tokens=* delims=" %%x in (rifleAnims.txt) DO (
...
set /a frameIndex=0
FOR /F "tokens=* delims=" %%x in (rifleAnimsFrames.txt) DO (
...
set /a fileIndex=0
FOR /F "delims=" %%x in (rifleAnimsFolders.txt) DO (
...
echo [1] for props (AR, shotgun, AK47)
echo [2] props (BP, beret)
